...
| Chat/summarize/translate routed through the vendor's own AI backend, on the vendor's terms      | Routed through _your_ AI provider, with _your_ key, under _your_ terms |
...
- **"Edit PDF" means three different things** across competitors: pdf.net edits _text runs in place_,
  iLovePDF/PDF24 add _annotation-layer objects_ (text boxes, shapes, images) on top of the page, and
  Adobe adds _comments_. We ship all three as one **Editor** host (T30) with a mode switch: **Direct
...
- **"Compress" vs "Optimize for web" vs "Repair".** PDF24 treats these as three tools; we keep them as
  three distinct routes because their _goals_ differ (smaller file vs. fast-first-byte-for-web vs.
...
| **pdf-lib**                                         | **MIT**                                                                                           | **Adopted** for PDF _writing/mutation_ — merge, split, page ops, forms, encryption, watermark, page numbers. Actively-maintained fork (`pdf-lib` successor, e.g. `@cantoo/pdf-lib`) pinned; ⚠ VERIFY current maintenance status at implementation time.                                    |
| **jsPDF**                                           | MIT                                                                                               | **Adopted** for programmatic PDF _creation_ (invoices, QR, blank templates) where pdf-lib's lower-level API is more code than needed.                                                                                                                                                      |
...
### 13.1.3 AI Justification Register (final Workstream E register)
...
| T29 (escalation only) PDF→Markdown restructuring        | inferring structure with no visual cues                          | Heuristics need _some_ signal (font size, indentation); absent that, judgement is required                             | Best-effort heuristic Markdown, clearly marked "structure inferred, review recommended" |
...
| T52 (escalation only) Alt-text authoring                | describing image _content_, not just flagging its absence        | No local model performs image captioning in this engine (parity with `image.complianttools.com` T71's same limitation) | A structural audit (missing tags, reading-order gaps) plus a manual-entry field         |
...
Workstream E reviewed every row above before adding adapter code. The register is closed for the
current build: no other code path may call a provider. T29, T44, T52, T59, and T61 remain optional
escalations owned by their local host tools; the shared escalation registry exposes their reason,
local-first remedy, capability, and gesture requirement. T65–T68 are the four user-facing AI flows.
Every external request is BYOK, prepared only after local work, cost-estimated, and confirmed by a
fresh click or keyboard gesture.
...
The engine exposes one provider-neutral `ProviderAdapter` interface (`chat`, `summarize`, `translate`,
`generate` capabilities). The three shipped adapter families are template-driven: the user supplies
the current endpoint, auth header, request template, and response path from the provider's own docs.
The app deliberately does not hardcode or claim a live vendor schema. Credentials are sent only as a
request header by the configured transport and never enter the JSON body, URL, recipe, logs, or
diagnostics.
...
A dedicated `/connect-ai` route explains BYOK in plain language, provides setup guidance for the
OpenAI-compatible, Anthropic-compatible, and generic HTTP families, and requires the user to supply
the current provider schema rather than relying on an unverified live assumption. Pricing is shown as
an estimate when the user configures rates; otherwise the UI says provider pricing is unknown. Keys
are saved only to IndexedDB after the save gesture, and the app recommends setting a spending cap in
...
PDFs are frequently much larger than a shareable URL, the recipe (the _steps_) is always shareable via
...
exists specifically to help _users of the tool_ produce more accessible PDFs — the tool itself must be
...
Unlike the image-toolkit sibling project, the _dominant_ open-source PDF rendering/editing engines
(MuPDF, Ghostscript) are AGPL, and the dominant _office-conversion_ engine (LibreOffice headless) is
...
- Digital-signature root-certificate trust lists — we verify against certificates the _user's own
  browser/OS_ trusts, or a user-supplied CA bundle; we never bundle or claim authority over a root
  program.
