...
| --- | --- | :-: | :-: | :-: |
| 0 | Bootstrap, shared contracts, toolchain, IP clearance | 16 | 0 | ⬜ |
...
- [ ] pnpm workspace, Node LTS pinned, `turbo.json` task graph (`build`/`test`/`lint`/`typecheck`)
- [ ] `tsconfig.base.json` strict mode
- [ ] Packages created empty: `engine`, `ui`, `cli`; apps: `web`, `relay`
- **Spec:** README §9 · **Done when:** `pnpm install && pnpm build` exits 0 on a clean clone

#### P0-02 · Lint, format, commit hygiene
- [ ] ESLint 9 flat config + Svelte plugin, Prettier, Commitlint + pre-commit hook
- [ ] **Custom rule:** no `eval`, `innerHTML`, `{@html}` outside a reviewed, sanitized allowlist
- [ ] **Custom rule:** no direct `fetch` in `packages/engine` outside `ai/transport.ts`
...
- [ ] Resolve full dependency graph at pinned versions
- [ ] Allowlist: MIT, Apache-2.0, BSD-2/3, ISC, Zlib, 0BSD, MPL-2.0, Unlicense, CC0
- [ ] **Deny** on GPL/LGPL/AGPL/SSPL/BUSL/non-commercial/unknown/missing — **with an explicit named
...
- [ ] Every asset in `apps/web/static/**` needs source URL, licence, licence URL, sha256, date checked
- [ ] Covers Tesseract `.traineddata` models, fonts, e-invoice XML schemas, ICC profiles
- **Spec:** README §25 · **Done when:** dropping an unregistered `.traineddata` file fails CI

#### P0-06 · Trademark / naming grep gate
- [ ] Fail the build on denied strings: specific e-signature vendor trademarks used as generic verbs,
...
- [ ] `docs/ADR/ip-clearance.md` created from README §25; every item resolved or explicitly excluded
...
- [ ] pdf.js (Apache-2.0) pinned and wrapped for parse/render/text-extraction, running in a worker
- [ ] pdfium WASM build pinned; licence of the **specific wrapper** verified and recorded (⚠ VERIFY,
  README §25.3)
...
- [ ] `pdf-lib` (or its actively-maintained fork) pinned for mutation: page ops, encryption, forms
- [ ] `jsPDF` pinned for from-scratch creation (invoices, blank templates, QR)
- **Spec:** README §7.2 · **Done when:** a worker merges two fixture PDFs and the result opens
  correctly in pdf.js

#### P0-10 · Core types and error model
- [ ] `PdfDocumentHandle`, `PageRef`, `Recipe`, `Step`, `EngineError` union — every variant carrying
  `remedy`
...
- [ ] Module workers, transferable buffers, `AbortSignal` cancellation observed within 50 ms
- **Spec:** README §8.4 · **Done when:** a test cancels a job mid-run and asserts abort < 50 ms

#### P0-12 · Design tokens + first UI primitives
- [ ] Ported from `image.complianttools.com` §12 tokens; Button, Slider, FileDrop, PageThumb
- **Spec:** README §12 · **Done when:** a visual test passes in both themes with no flash on load

#### P0-13 · Adversarial PDF corpus (seed set)
- [ ] Truncated file, circular object refs, 0-page and 1,000,000-declared-page files, malformed
...
- [ ] `no-network` Playwright harness scaffolded, passing on a trivial flow
- [ ] `credential-leak` harness asserting no key value can reach a log/error/diagnostic bundle
- **Spec:** README §22, §16 · **Done when:** both exist as CI jobs and pass

#### P0-15 · Plan-sync gate
- [ ] `scripts/check-plan-sync.ts` fails CI when README.md changes without a PLAN.md change
- **Spec:** this file §0.3 · **Done when:** a README-only PR fails, and passes once PLAN.md is updated

#### P0-16 · Page delivery architecture
- [ ] `adapter-static`, `prerender = true` on all indexable routes; real `<input type="file">` in
...
- [ ] `verify:licenses` runs on every CI run; **no AGPL/GPL dependency exists in the lockfile**,
  verified by the named MuPDF/Ghostscript-rejection test
- [ ] Static-asset and trademark gates run on every CI run
- [ ] `docs/ADR/ip-clearance.md` has a decision or explicit fallback for every §25.3 item
- [ ] A worker merges two fixture PDFs and pdf.js opens the result correctly
- [ ] Adversarial seed corpus: zero crashes, zero embedded-JS execution, every error typed with a remedy
- [ ] `no-network`, `credential-leak`, and `plan-sync` exist as CI jobs and pass
